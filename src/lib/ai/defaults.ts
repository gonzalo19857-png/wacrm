...
export function buildSystemPrompt(args: {
  ...
  if (mode === 'auto_reply') {
    ...
    parts.push(
      `If the business context below defines a process for collecting delivery/shipping details (an agency or address, a recipient name, DNI, phone, a delivery time slot, etc.), follow that process, and record whatever you confirm along the way by emitting it on its own line as ${SHIPMENT_SENTINEL_PREFIX}field=value;field2=value2]] using only these keys: region (lima or provincia), city, agency, name, dni, phone, address, reference, notes (free text — e.g. a chosen delivery slot/day). Every value must be the literal, real value the customer gave or confirmed — never a placeholder, a description, or an explanation of why you don't have it (if you don't have a real value for a key, omit that key entirely rather than writing something like "sin especificar" or a bracketed placeholder). Include only fields you actually learned or confirmed this turn — never invent a value for any of them (an agency name, an address, a reference point — nothing you weren't explicitly told by the customer or given verbatim in this system prompt), and don't re-send a field already on file (see "Current shipment on file" below) unless it changed. Map by MEANING, not by the order the customer typed things in: when a customer sends several pieces of data in one message (with or without their own labels, e.g. "NOMBRE=...", a city, an agency, all on separate lines), read the whole message and match each value to the right key by what it actually is — \`name\` is always a person's full name, never a place; \`city\`/\`agency\`/\`address\` are always places, never a person's name. Double-check before emitting: a value that looks like a place (has a department/city name, or matches something already in \`city\`) must never end up in \`name\`. This sentinel is invisible to the customer: never mention it or read its contents back to them. If "Current shipment on file" below already shows a field, don't ask for it again — only ask for what's still missing, and if the customer asks about their order's status (e.g. "¿ya llegó?"), answer directly from its \`status\` there instead of guessing or handing off.`,
    )
    parts.push(
